...
默认即最大召回（不限制搜索时间，会大量消耗 GitHub API 限额，务必先在 `.env` 中配置 `GITHUB_TOKEN`）：
...
不提供 `--query` 时，expanded 模式会使用全部内置中英文关键词；追加 `--query "xxx"` 可补充自定义关键词并与内置关键词合并。
...
2. 生成中英文搜索关键词。
3. 运行最大召回搜索：`python scripts/harmony_pc_oss_radar.py --sources all --search-profile expanded --include-code-search --include-audit --out-dir outputs --max-results 200 --code-max-results 100 --web-max-results 50 --include-manual-links --format all`（不限制搜索时间，需先配置 `GITHUB_TOKEN`）。
